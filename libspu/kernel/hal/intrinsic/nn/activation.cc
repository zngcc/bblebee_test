...
static Value do_f_seg3_gelu(SPUContext* ctx, Value x, bool small_ring_compare) {
  ...
  // 定义区间划分点（转换为固定点）
  const float seg1_boundary = -5.0;
  const float seg2_boundary = -1.97;
  const float seg3_boundary = 3.0;
  ...
  // 计算比较结果，得到三个边界条件
  auto batch_less_than =
      ComputedBatchLessAP(ctx, x, {seg1_boundary, seg2_boundary, seg3_boundary});
  
  // 计算四个区间的掩码
  // b0: x < -5
  auto b0 = batch_less_than[0];
  auto t0 = _xor(ctx, batch_less_than[0], ONE);
  auto t1 = _xor(ctx, batch_less_than[1], ONE);
  auto t2 = _xor(ctx, batch_less_than[2], ONE);



  // b1: -5 <= x < -1.97
  auto b1 = _xor(ctx, t0, t1);
  // b2: -1.97 <= x < 3
  auto b2 = _xor(ctx, t1, t2);
  // b3: x >= 3
  auto b3 = _xor(ctx, batch_less_than[2], ONE);

  // 计算x^2，用于二次多项式
  auto x2 = f_square(ctx, x);
  
  // 定义第一个区间 (-∞, -5) 的系数：常数 -e (e = 0.00001)
  // 注意：这里的负号是Python代码中的z_*(-e)
  const float seg0_e = 0.00001;
  const float seg0_coeff = -seg0_e;
  auto seg0 = constant(ctx, seg0_coeff, x.dtype(), x.shape());

  // 第二个区间 [-5, -1.97) 的系数：deg=2 多项式
  // Python代码中的系数：a = [-0.1508, -0.0763, -0.0095]
  // 多项式形式：a0 + a1*x + a2*x^2
  const float seg1_a0 = -0.1508;
  const float seg1_a1 = -0.0763;
  const float seg1_a2 = -0.0095;
  
  // 计算 seg1_a2 * x^2
  auto seg1_part2 = _mul(ctx, x2, constant(ctx, seg1_a2, x.dtype(), x.shape()));
  // 计算 seg1_a1 * x
  auto seg1_part1 = _mul(ctx, x, constant(ctx, seg1_a1, x.dtype(), x.shape()));
  // 加上常数项
  auto seg1_part0 = constant(ctx, seg1_a0, x.dtype(), x.shape());
  
  auto seg1_untrunc = _add(ctx, seg1_part0,
                         _add(ctx, seg1_part1, seg1_part2));
  auto seg1 = _trunc(ctx, seg1_untrunc);

  // 第三个区间 [-1.97, 3) 的系数：deg=2 多项式
  // Python代码中的系数：b = [0.1227, 0.4772, 0.1897]
  // 多项式形式：b0 + b1*x + b2*x^2
  const float seg2_b0 = 0.1227;
  const float seg2_b1 = 0.4772;
  const float seg2_b2 = 0.1897;
  
  // 计算 seg2_b2 * x^2
  auto seg2_part2 = _mul(ctx, x2, constant(ctx, seg2_b2, x.dtype(), x.shape()));
  // 计算 seg2_b1 * x
  auto seg2_part1 = _mul(ctx, x, constant(ctx, seg2_b1, x.dtype(), x.shape()));
  // 加上常数项
  auto seg2_part0 = constant(ctx, seg2_b0, x.dtype(), x.shape());
  
  auto seg2_untrunc = _add(ctx, seg2_part0,
                         _add(ctx, seg2_part1, seg2_part2));
  auto seg2 = _trunc(ctx, seg2_untrunc);

  // 第四个区间 [3, ∞) 的系数：x - e (e = 0.00001)
  const float seg3_e = 0.00001f;
  auto seg3 = _add(ctx, x, constant(ctx, -seg3_e, x.dtype(), x.shape()));

  // 组合四个区间的结果
  auto gelu = _mul(ctx, b0, seg0);
  gelu = _add(ctx, gelu, _mul(ctx, b1, seg1));
  gelu = _add(ctx, gelu, _mul(ctx, b2, seg2));
  gelu = _add(ctx, gelu, _mul(ctx, b3, seg3));
  gelu.setDtype(x.dtype());
