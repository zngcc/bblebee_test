...
parser.add_argument("-m", "--model_path", default="./bert.pth", help="Path to .pth model")
...
def run_on_spu(model, input_ids, attention_masks, token_type_ids, labels):
    ...
    spu_token_type_ids = ppd.device("P1")(lambda x: x)(token_type_ids)
    spu_params = ppd.device("P2")(lambda x: x)(params)
    
    start = time.time()
    logits_spu = ppd.device("SPU")(eval, copts=copts)(
        spu_params, spu_input_ids, spu_attention_masks, spu_token_type_ids
    )
    end = time.time()
